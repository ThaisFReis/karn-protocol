...
use soroban_sdk::{testutils::{Address as _, Ledger}, Address, Env, IntoVal, Symbol, BytesN};
...
fn test_integration_full_flow() {
    ...
    val.initialize(&founder, &gov_id, &tr_id, &member_valor_id, &ids, &rarities, &metas, &founder_valor_id, &signer);
    ...
    env.ledger().set_timestamp(current_time + voting_delay + voting_period + 1);
